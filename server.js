...
// Multer - memoria (salva base64 no banco)
const upload = multer({
...
app.post('/api/admin/products', auth, async (req, res) => {
    try {
          ...
          const { rows } = await pool.query(
                  `INSERT INTO products (name, category, price, description, available) VALUES ($1, $2, $3, $4, $5) RETURNING *`,
...
app.put('/api/admin/products/:id', auth, async (req, res) => {
    try {
          const { name, category, price, description, available } = req.body;
          const { rows } = await pool.query(
                  `UPDATE products SET name=$1, category=$2, price=$3, description=$4, available=$5 WHERE id=$6 RETURNING *`,
...
app.post('/api/admin/products/:id/images', auth, upload.single('image'), async (req, res) => {
    try {
          ...
          if (parseInt(count.rows[0].count) >= 4) {
                  return res.status(400).json({ error: 'Maximo de 4 fotos por produto' });
          }
...
// PUT /api/admin/settings/banner-image
